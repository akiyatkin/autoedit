...
jsonwidget.localstrings = {
	'ru': {'help': '?',
		...
		'Add': 'Ещё',
		'to': 'к',
	"required attribute - deletion not allowed":"Обязательный параметр нельзя удалить",
	'extension field': 'добавить'
	}
};
...
jsonwidget.jsonTreeRef.attachSchema = function () {
	if(this.schemaref.node.type == 'any') {
		if(this.getType()=='map') {
			...
			this.schemaref.node.user_key = _("extension");
		}
	}
}
...
jsonwidget.editor = function () {
	...
	this.getMoveButton = jsonwidget.editor.getMoveButton;
...
jsonwidget.editor.attachHandlers = function () {
	...
	this.formdiv[document.attachEvent?'attachEvent':'addEventListener'](document.attachEvent?'onchange':'change',onchange);
	...
	this.formdiv.onclick = function (event) {
		...
		/*if(target.className == "je_movedown") {
		var jsonindex = target.id.substr("je_movedown".length-2);
		var jsonnode = jsoneditobj.jsonLookupById[jsonindex];
		var parent = jsonnode.parent;
		jsonnode.domparent.parentNode.removeChild(jsonnode.domparent);
		if(parent) {
			if(parent.node instanceof Array) {
				var node=parent.node.splice(jsonnode.nodeindex,1);
				parent.node.splice(jsonnode.nodeindex,0,node[0]);
			}
			jsoneditobj.updateNode(parent);
		}

		jsonwidget.movingnow=true;
		clearTimeout(jsonwidget.movingtime);
		jsonwidget.movingtime=setTimeout(function(){
			jsonwidget.movingnow=false;
		},1000);
	}*/

		if(target.className == "je_moveup") {
		jsonwidget.movingnow=true;
		clearTimeout(jsonwidget.movingtime);
		jsonwidget.movingtime=setTimeout(function(){
			jsonwidget.movingnow=false;
		...
		var jsonindex = target.id.substr("je_moveup".length-2);
		var jsonnode = jsoneditobj.jsonLookupById[jsonindex];
		var parent = jsonnode.parent;
		jsonnode.domparent.parentNode.removeChild(jsonnode.domparent);
		if(parent) {
			if(parent.node instanceof Array) {
				var node=parent.node.splice(jsonnode.nodeindex,1);
				parent.node.splice(jsonnode.nodeindex-1,0,node[0]);
			}
			jsoneditobj.updateNode(parent);
		}
		}
	...
	/*if(jsonwidget.movingnow)return;
	if(document.getElementsByClassName){
	var els=document.getElementsByClassName('je_hide');
	for(var i=1,l=els.length;i<l;i++){
		jsonwidget.fireEvent(els[i],'click');
	}
	}*/

}
jsonwidget.hideNode=function(nodeid){
	var t=document.getElementById("contents."+nodeid);
	t.style.display="none";
	var a=t.nextSibling||t.nextElementSibling;
	if(a)a.style.display="none";
	document.getElementById("hide."+nodeid).style.display="none";
	document.getElementById("show."+nodeid).style.display="inline";
};
jsonwidget.showNode=function(nodeid){
	var t=document.getElementById("contents."+nodeid);
	t.style.display="";
	var a=t.nextSibling||t.nextElementSibling;
...
jsonwidget.editor.getArrayInputAttrs = function (jsonref) {
	...
	retval.className="jsonedit_tbl";
	retval.id=contentsid;

	var tbody = document.createElement("tbody");

	for(var i in jsonref.node) {
	if(!jsonref.node.hasOwnProperty(i))continue;
		var rownode = document.createElement("tr");
...
jsonwidget.editor.getMoveButton = function (jsonref,derection) {
	var hideid='move'+derection+'.'+jsonref.fullindex;
	var retval = document.createElement("span");

	retval.setAttribute("title", _("move"));

	retval.className="je_move"+derection;
	if(derection=='up'&&jsonref.nodeindex==0){
		retval.className='je_disabled';
		retval.setAttribute("title", _("Поднять выше нельзя"));
	}
	if(derection=='down'&&jsonref.nodeindex==jsonref.parent.node.length-1){
		retval.className='je_disabled';
		retval.setAttribute("title", _("Опустить ниже нельзя"));
	}
	retval.id=hideid;
	retval.appendChild(document.createTextNode("[ "+_("move"+derection)+" ]"));
	return retval;
}
...
jsonwidget.editor.getDeleteButton = function (jsonref) {
	...
	if(!jsonref.schemaref.node.required) {
		...
		retval.className="je_disabled";
		retval.setAttribute("title", _("required attribute - deletion not allowed"));
	}
	retval.id=rmid;
...
jsonwidget.editor.attachArrayInput = function (jsonref) {
	...
	var s=jsonref.schemaref.node;
	if(jsonref.parent)var p=jsonref.parent.schemaref.node;
	else p=false;
	if(s.desc)localnode.appendChild(this.getHelpButton(jsonref));
	//localnode.appendChild(this.getHideButton(jsonref));
	//localnode.appendChild(this.getShowButton(jsonref));
	if(jsonref.parent&&jsonref.parent.node instanceof Array){
		localnode.appendChild(this.getMoveButton(jsonref,'up'));
		//localnode.appendChild(this.getMoveButton(jsonref,'down'));
	}
...
jsonwidget.editor.attachSimplePropertyInput = function (jsonref) {
	...
	if(jsonref.domparent.tagName == 'TR') {
		...
		if(jsonref.getType() == undefined ) {
			namenode.appendChild(document.createTextNode(_("value type")+": "));
		}
		else {
	...
	var s=jsonref.schemaref.node;
	if(s.desc)controldiv.appendChild(this.getHelpButton(jsonref));
...
jsonwidget.editor.getStringInput = function (jsonref) {
	...
	if(jsonref.schemaref.node['enum']==undefined) {
		...
		for(var i in jsonref.schemaref.node['enum']) {
			var val=jsonref.schemaref.node['enum'][i];
		...
		if(!validvalue) {
			var first='';
			for(first in jsonref.schemaref.node['enum'])break;
			this.setNode(jsonref, first);
		}
	}
	return inputelement;
}
...
jsonwidget.editor.getTypeSelectInput = function (jsonref) {
	...
	var types = [['Тип','any'],['Строка','str'],['Число','int'],['Boolean','bool'],['Массив','seq'],['Объект','map']];
	...
	for(var i=0,l=types.length;i<l;i++) {
		var option = document.createElement("option");
	option.value=types[i][1];
		option.appendChild(document.createTextNode(types[i][0]));
		inputelement.appendChild(option);
	}
	return inputelement;
}
...
jsonwidget.editor.setView = function setView (viewtoset) {
	...
	for(var i=0,l=this.views.length;i<l;i++) {
		...
		if(viewtoset == this.views[i]) {
			...
			case 'form':
				...
				break;
			case 'source':
		if(this.textarea)var ta=this.textarea;
		else var ta=document.getElementById(this.htmlids.sourcetextarea);
		ta.style.display="none";
				break;
			case 'schemasource':
				if(!this.schema)document.getElementById(this.htmlids.schematextarea).style.display="none";
				break;
			case 'schemaform':
	...
	case "source":
		...
		if(je.textarea)var ta=je.textarea;
		else var ta=document.getElementById(je.htmlids.sourcetextarea);
	...
	case "schemasource":
		setTimeout(function () {
			je.updateJSON();
			if(!this.schema)document.getElementById(je.htmlids.schematextarea).style.display="inline";
			je.clearStatusLight();
...
jsonwidget.fireEvent=function(element,event){
...
jsonwidget.editor.updateJSON = function () {
	...
	var old=jsonarea.value;
	/*var parent = jsonarea.parentNode;
	var nextsibling = jsonarea.nextSibling;
	parent.removeChild(jsonarea);*/
	jsonarea.innerHTML=jsonarea.value=str;
	if(old!=str) jsonwidget.fireEvent(jsonarea,'change');
...
jsonwidget.editor.setFormOnSubmit = function () {
	...
	var je = this;
	var sourcetextform = this.textarea.form;
	sourcetextform[document.attachEvent?'attachEvent':'addEventListener'](document.attachEvent?'onsubmit':'submit',function(){
		if(je.currentView != 'source') {
			var val=je.updateJSON();
		}
	});
	this.textarea[document.attachEvent?'attachEvent':'addEventListener'](document.attachEvent?'onchange':'change',function(){
		setTimeout(function(){
			if(je.currentView=='form'){
				je.toggleToFormActual();
			}
		},1);
	});
}
window.jsonedit=function(ta,schema){
	if(!schema)schema={};
	var run=function(obj,callback){
		//if(obj.reqursion)return;
		if(obj.properties) for(var i in obj.properties) run(obj.properties[i],callback);
		if(obj.items) run(obj.items,callback);
		callback(obj);
	}
	var ids={};
	run(schema,function(el){
		if(el['$ref']){
			el['idref']=el['$ref'];
			el['type']='idref';
		}
		if(el.properties){
			el.mapping=el.properties;
			delete el.properties;
		}
		if(el.type=='string')el.type='str';
		if(el.type=='object')el.type='map';
		if(el.type=='array')el.type='seq';
		if(el.description)el.desc=el.description;

		if(el.type=='seq'&&!el.sequence&&!el.items){
			el.items={type:'any'}
		}
		if(el.items){
			el.sequence=[el.items];
			delete el.items;
		}
		if(el.type=='map'){
			if(!el.mapping){
				if(!el.user_key)el.user_key='new';
				el.mapping={}
				el.mapping[el.user_key]={title:'Новое свойство',type:'any'};
			}
		}
		if(!el.type)el.type='any';
	});
	window.sss=schema;
	var form=ta.form;
	...
	div.innerHTML='<div id="je_warningdiv"></div><div id="je_buttondiv"><span id="je_formbutton" style="cursor: pointer"> JSON Редактор&nbsp;</span><span id="je_sourcebutton" style="cursor: pointer">&nbsp;Исходный код </span></div><div id="je_formdiv" style="text-background: white"> </div>';
	//form.parentNode.insertBefore(div,form);
	ta.parentNode.insertBefore(div,ta);

	form[document.attachEvent?'attachEvent':'addEventListener'](document.attachEvent?'onsubmit':'submit',function(){
		if(je.currentView != 'source') {
			je.updateJSON();
		}
	});
