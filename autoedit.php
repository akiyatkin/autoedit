...
infra_require('*autoedit/admin.inc.php');
	...
	if (infra_admin()) {
		if (in_array($type, array('mvdir', 'mkdir', 'cpdir', 'rmdir'))) {
			if ($id{0} != '*') {
				return infra_echo($ans, 'Путь должен быть тематический, начинаться с *.');
			}

			if ($type == 'mvdir') {
				$ans['msg'] = 'Ошибка перемещения директории!';
			}//id папка которую нужно перенести
			if ($type == 'cpdir') {
				$ans['msg'] = 'Ошибка копирования директории!';
			}//id папка которую нужно скопировать
			if ($type == 'rmdir') {
				$ans['msg'] = 'Ошибка удаления директории!';
			}//id папка которую нужно удалить 
			if ($type == 'mkdir') {
				$ans['msg'] = 'Ошибка создания директории!';
			}//id папка в которой нужно создать!!!

			if ($type === 'mkdir' && !infra_theme($id, 'snd')) {
				return infra_echo($ans, 'Нет папки в которой нужно создать');
			}
			if ($type === 'mvdir' && !infra_theme($id, 'snd')) {
				return infra_echo($ans, 'Нет папки которую нужно перенести');
			}
			if ($type === 'cpdir' && !infra_theme($id, 'snd')) {
				return infra_echo($ans, 'Нет папки которую нужно скопировать');
			}
			if ($type === 'rmdir' && !infra_theme($id, 'snd')) {
				return infra_echo($ans, 'Нет папки которую нужно удалить');
			}
			...
			if (!$submit) {
				...
				if (in_array($type, array('mvdir', 'cpdir', 'rmdir'))) {
					...
					if ($id !== $oldpath) {
						return infra_echo($ans, 'Ошибка в переданных параметрах');
					}
					if (!infra_theme($oldpath, 'snd')) {
						return infra_echo($ans, 'Не найден оригинальный путь');
					}
				}
				if (in_array($type, array('mvdir', 'cpdir', 'mkdir'))) {
					...
					if (!$newname) {
						return infra_echo($ans, 'Нужно указать имя');
					}
					if (preg_match('/\//', $newname)) {
						return infra_echo($ans, 'Имя папки не может содержать слэш');
					}
					if (!infra_theme($newfolder, 'snd')) {
						return infra_echo($ans, 'Не найдено новое место где нужно расположить папку');
					}
					if (infra_theme($newpath, 'snd')) {
						return infra_echo($ans, 'Такая папка уже существует или имя занято');
					}
				}
				if ($type === 'mvdir') {
...
$file = autoedit_createPath($origfile);
					...
					if (!$file) {
						return infra_echo($ans, 'Не удалось создать путь до файла '.infra_toutf($file));
					}
				}
			...
			if (!$submit) {
				...
				if ($r) {
					return infra_echo($ans, 'SEO-данные сохранены', 1);
				}
			...
			if (!$submit) {
				...
				if (!$isfile) {
					...
					if (!$file) {
						return infra_echo($ans, 'Не удалось создать путь до файла'.$id);
					}
				} else {
					$file = $isfile;
				}
